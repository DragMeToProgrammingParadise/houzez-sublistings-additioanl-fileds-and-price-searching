...
<div id="sub-properties" class="dashboard-content-block-wrap <?php echo esc_attr($is_multi_steps);?>">
    ...
    <?php if ( isset( $multi_unit['fave_mu_payment_plans'] ) && is_array( $multi_unit['fave_mu_payment_plans'] ) ) : ?>
        <?php foreach ( $multi_unit['fave_mu_payment_plans'] as $plan_index => $payment_plan ) : ?>
            <!-- Editing Payment Plan Fields -->
            <div class="payment-plan">
                <div class="form-group">
                    <label for="fave_multi_units[<?php echo intval( $count ); ?>][fave_mu_payment_plans][<?php echo intval( $plan_index ); ?>][payment_title]">Title</label>
                    <input value="<?php echo sanitize_text_field( $payment_plan['payment_title'] ); ?>" name="fave_multi_units[<?php echo intval( $count ); ?>][fave_mu_payment_plans][<?php echo intval( $plan_index ); ?>][payment_title]" type="text" class="form-control" placeholder="Enter Payment Plan Title">
                </div>

                <div class="form-group">
                    <label for="fave_multi_units[<?php echo intval( $count ); ?>][fave_mu_payment_plans][<?php echo intval( $plan_index ); ?>][payment_value]">Value</label>
                    <input value="<?php echo sanitize_text_field( $payment_plan['payment_value'] ); ?>" name="fave_multi_units[<?php echo intval( $count ); ?>][fave_mu_payment_plans][<?php echo intval( $plan_index ); ?>][payment_value]" type="text" class="form-control" placeholder="Enter Payment Plan Value">
                </div>
            </div>
        <?php endforeach; ?>
    <?php endif; ?>

    <!-- Add button for adding new payment plans dynamically while editing -->
    <button type="button" class="btn btn-primary add-payment-plan" data-unit="<?php echo intval($count); ?>">Add Payment Plan</button>
</div>
    ...
    <!-- Initial Payment Plan Fields -->
    <div class="payment-plan">
        <div class="form-group">
            <label for="fave_multi_units[0][fave_mu_payment_plans][0][payment_title]">Title</label>
            <input name="fave_multi_units[0][fave_mu_payment_plans][0][payment_title]" type="text" class="form-control" placeholder="Enter Payment Plan Title">
        </div>

        <div class="form-group">
            <label for="fave_multi_units[0][fave_mu_payment_plans][0][payment_value]">Value</label>
            <input name="fave_multi_units[0][fave_mu_payment_plans][0][payment_value]" type="text" class="form-control" placeholder="Enter Value for Payment Plan">
        </div>
    </div>

    <!-- Add button for adding new payment plans dynamically -->
    <button type="button" class="btn btn-primary add-payment-plan" data-unit="0">Add Payment Plan</button>
</div>
